...
export const textEncode = (value) => new Uint8Array(Buffer.from(value, 'utf8'));
export const textDecode = (buffer) => Buffer.from(buffer).toString('utf8');
/**
 * @description Convert from base58 to integer
 * @param {string} v The b58 value
 * @returns {string} The converted b58 value
 */
export const b582int = (v) => {
 ...
 */
export const mutez = (tez) => new BigNumber(new BigNumber(tez).toFixed(6)).multipliedBy(1000000).toString();
/**
 ...
 */
export const b58cencode = (payload, prefixArg) => {
 ...
 */
export const b58cdecode = (enc, prefixArg) => bs58check.decode(enc).slice(prefixArg.length);
/**
...
export const mic2arr = function me2(s) {
    let ret = [];
...
export const ml2mic = function me(mi) {
    const ret = [];
    let inseq = false;
    let seq = '';
    let val = '';
